...
curl -i https://news-crypto.vercel.app/api/premium/ai/signals?coin=bitcoin
...
curl -H "X-Payment: <payment_proof>" \
  https://news-crypto.vercel.app/api/premium/ai/signals?coin=bitcoin
```
...
```bash
# Discovery endpoint
curl https://news-crypto.vercel.app/.well-known/x402.json
```
