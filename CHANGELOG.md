...
- **[x402 conformance auditor](docs/x402-conformance.md)** - a new tool, endpoint and page that catch the class of x402 bug nothing else looks for. A paid API states its price twice, in two different units, written by two different pieces of code: decimal USD in the discovery document an agent reads before calling, and token atomic units in the `402` challenge it gets at the door. Every existing validator checks each side alone and reports both green while they describe different prices, different protection, or different call shapes.
  - **[`@nirholas/x402-conformance`](sdk/x402-conformance)** - the engine, as a dependency-free package with a CLI. `npx @nirholas/x402-conformance <origin>`. Emits a graded report, JSON, or SARIF 2.1.0 for code scanning; `--fail-on` gates a deploy and `--baseline` catches a regression between two deploys. Nothing it does spends money: every probe is an ordinary unpaid request.
  - **Cross-validation** - the two prices are checked without an asset registry, by recovering the token's decimals from the ratio: an honest pair differs by exactly a power of ten, so a pair that does not is a real disagreement rather than an unknown token. Also checks that paid means paid and free means free, that a dynamic quote stays under its own advertised ceiling, that a fixed quote does not move between calls, and that the payment gate runs before argument validation.
  - **Payability** - a challenge whose `payTo` is the zero address validates against every schema in existence and burns the funds of the first agent that honours it. Nothing else checks for it; this does, and it caps the grade when it finds one.
  - **[/x402/conformance](https://cryptocurrency.cv/x402/conformance)** and **`GET /api/x402/conformance`** - the same engine, hosted. Audit any public origin, get the finding and the fix. The page and the npm package share one implementation, so a grade from the browser and a grade from a terminal cannot disagree.
