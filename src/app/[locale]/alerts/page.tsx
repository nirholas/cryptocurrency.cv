...
const ALERT_TYPE_OPTIONS: {
  ...
  },
  {
    ...
    label: 'Volume Spike',
    icon: BarChart3,
    description: 'Alert when 24h volume exceeds target',
  },
  {
    value: 'recurring',
    label: 'Recurring',
    icon: RefreshCw,
    description: 'Re-fires after cooldown period',
  },
];
...
export default function AlertsPage() {
  ...
  const typeIcon = (type: AlertType) => {
    switch (type) {
      ...
      case 'percent_change':
        return <Percent className="text-accent h-4 w-4" />;
      case 'volume_spike':
  ...
  const SortHeader = ({ field, children }: { field: SortField; children: React.ReactNode }) => (
    <th
      className="hover:text-text-primary cursor-pointer pr-4 pb-3 font-medium transition-colors select-none"
    ...
    <>
      ...
      <main className="container-main py-10">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-text-primary mb-2 flex items-center gap-3 font-serif text-3xl font-bold md:text-4xl">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="border-border rounded-lg border bg-(--color-surface) p-3 text-center">
            <p className="text-text-primary text-2xl font-bold tabular-nums">{stats.total}</p>
            <p className="text-text-tertiary text-xs">Total Alerts</p>
          </div>
          <div className="border-border rounded-lg border bg-(--color-surface) p-3 text-center">
          ...
          <div className="border-border rounded-lg border bg-(--color-surface) p-3 text-center">
            <p className="text-accent text-2xl font-bold tabular-nums">{stats.triggered}</p>
            <p className="text-text-tertiary text-xs">Triggered</p>
          </div>
          <div className="border-border rounded-lg border bg-(--color-surface) p-3 text-center">
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <label className="text-text-secondary mb-1 block text-sm font-medium">Coin</label>
                  <select
                    value={coinId}
                    onChange={(e) => setCoinId(e.target.value)}
                    className="border-border focus:ring-accent w-full rounded-md border bg-(--color-surface) px-3 py-2 text-sm focus:ring-2 focus:outline-none"
                  >
                    {COIN_OPTIONS.map((c) => (
                  ...
                  {livePrices[coinId]?.usd && (
                    <p className="text-text-tertiary mt-1 text-xs">
                      ...
                      <span className="text-text-primary font-medium">
                        ${livePrices[coinId].usd.toLocaleString()}
                      </span>
                        ...
                        {[0.95, 0.9, 1.05, 1.1].map((mult) => {
                          ...
                          return (
                            <button
                              ...
                              className="border-border text-text-tertiary hover:border-accent hover:text-accent rounded border px-2 py-0.5 text-[10px] font-medium transition-colors"
                            >
                              {mult < 1
                ...
                <div className="space-y-2">
                  {[
                    ...
                    },
                    {
                      ...
                      name: 'Bitcoin',
                      target: 5,
                      type: 'percent_change' as AlertType,
                      label: 'BTC ±5% day',
                    },
                    {
                      coin: 'solana',
                    ...
                    const alreadyExists = alerts.some(
                      (a) =>
                        a.coinId === preset.coin &&
                        a.type === preset.type &&
                        a.target === preset.target,
                    );
                    return (
                      <button
                        key={i}
                ...
                return (
                  ...
                  >
                    ...
                    {tab.count !== null && tab.count > 0 && (
                      <span className="bg-accent/10 text-accent ml-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold tabular-nums">
                        {tab.count}
                      </span>
                    )}
                  </button>
                ...
                <CardContent className="pt-4">
                  ...
                  {alerts.length > 0 && (
                    <div className="mb-4 flex flex-col gap-3">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="relative min-w-50 flex-1">
                          ...
                          <input
                            ...
                            className="border-border placeholder:text-text-tertiary focus:ring-accent/40 w-full rounded-md border bg-(--color-surface) py-2 pr-3 pl-9 text-sm focus:ring-2 focus:outline-none"
                          />
                        </div>
                        ...
                        <select
                          ...
                          className="border-border focus:ring-accent rounded-md border bg-(--color-surface) px-3 py-2 text-sm focus:ring-2 focus:outline-none"
                        >
                      ...
                      {/* Bulk actions */}
                      {selectedIds.size > 0 && (
                        <div className="bg-accent/5 border-accent/20 flex items-center gap-2 rounded-md border px-3 py-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => bulkToggle([...selectedIds], true)}
                          >
                            Enable All
                          </Button>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => bulkToggle([...selectedIds], false)}
                          >
                            Disable All
                          </Button>
                          <Button
                  ...
                  {filteredAlerts.length === 0 ? (
                    <div className="space-y-3 py-12 text-center">
                      <Bell className="text-text-tertiary mx-auto h-12 w-12" />
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-border text-text-secondary border-b text-left">
                            ...
                            <th className="pr-4 pb-3 font-medium">Now</th>
                            ...
                            >
                              <td className="py-3 pr-2">
                                <button onClick={() => toggleSelect(alert.id)}>
                                  {selectedIds.has(alert.id) ? (
                                    ...
                                    <Square className="text-text-tertiary h-4 w-4" />
                                  )}
                                </button>
                              </td>
                              <td className="py-3 pr-4">
                                <div>
                                  ...
                                  {alert.note && (
                                    <p className="text-text-tertiary max-w-30 truncate text-[10px]">
                                      {alert.note}
                                    </p>
                                  )}
                                </div>
                              ...
                              <td className="text-text-secondary py-3 pr-4 tabular-nums">
                                {currentPrice(alert.coinId)}
                              </td>
                              <td className="py-3 pr-4">{priorityBadge(alert.priority)}</td>
                              <td className="py-3 pr-4">
                                <button
            ...
            {activeTab === 'triggered' && (
              <Card>
                ...
                <CardContent>
                  {triggered.length === 0 ? (
                    <div className="space-y-3 py-12 text-center">
                      <Clock className="text-text-tertiary mx-auto h-12 w-12" />
                      <h3 className="text-text-primary text-lg font-semibold">
                        No triggered alerts yet
                      </h3>
                      <p className="text-text-secondary mx-auto max-w-md text-sm">
                        When your price alerts fire, they&apos;ll appear here with timestamp and
                        price details.
                      </p>
                    </div>
                  ) : (
                    <ul className="space-y-2">
                      {triggered.map((t, i) => (
                        <li
                          key={`${t.id}-${i}`}
                          className="border-border hover:border-accent/30 group flex items-start gap-3 rounded-lg border p-3 text-sm transition-colors"
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <button
                                ...
                                className="text-text-tertiary opacity-0 transition-opacity group-hover:opacity-100 hover:text-red-500"
                                title="Remove"
                              >
                  ...
                  {Object.keys(livePrices).length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-border text-text-secondary border-b text-left">
                            <th className="pr-4 pb-3 font-medium">Coin</th>
                            <th className="pr-4 pb-3 text-right font-medium">Price</th>
                            <th className="pr-4 pb-3 text-right font-medium">24h Change</th>
                            <th className="pb-3 text-right font-medium">Active Alerts</th>
                          </tr>
                        </thead>
                        <tbody>
                          {Object.entries(livePrices).map(([id, data]) => {
                            ...
                            const alertCount = alerts.filter(
                              (a) => a.coinId === id && a.enabled,
                            ).length;
                            const change = data.usd_24h_change ?? 0;
                            return (
                              <tr key={id} className="border-border border-b last:border-0">
                                <td className="text-text-primary py-3 pr-4 font-medium">
                                  {coinMeta?.name ?? id}
                                  {coinMeta && (
                                ...
                                <td className="text-text-primary py-3 pr-4 text-right font-medium tabular-nums">
