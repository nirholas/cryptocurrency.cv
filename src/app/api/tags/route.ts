...
export const revalidate = 300; // ISR: tags refresh every 5 min
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (slug) {
      ...
      if (!tag) {
        return NextResponse.json({ error: 'Tag not found', slug }, { status: 404 });
      }
    ...
    if (category) {
      const validCategories: Tag['category'][] = [
        'asset',
        'topic',
        'event',
        'technology',
