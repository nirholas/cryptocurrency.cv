...
function calculateConsistency(
  ...
  const clickbaitVariance =
    clickbaitScores.reduce((sum, score) => sum + Math.pow(score - clickbaitMean, 2), 0) /
  ...
  const sentimentVariance =
    sentiments.reduce((sum, score) => sum + Math.pow(score - sentimentMean, 2), 0) /
    sentiments.length;
 ...
 */
function calculateBias(articles: Array<{ sentiment: number }>): BiasMetrics {
...
export function calculateSourceCredibility(sourceKey: string): SourceCredibility | null {
  ...
  const timeliness = articles.length >= 2 ? calculateTimeliness(articles) : baseTimeliness;

  const consistency = articles.length >= 3 ? calculateConsistency(articles) : baseConsistency;
...
export async function getSourceCredibility(options?: {
  ...
  return withCache(newsCache, cacheKey, 3600, async () => {
    ...
    const averageScore =
      sources.length > 0 ? sources.reduce((sum, s) => sum + s.overallScore, 0) / sources.length : 0;

    return {
      sources,
      averageScore: parseFloat(averageScore.toFixed(1)),
      topSources: sources.slice(0, 3).map((s) => s.source),
      bottomSources: sources
        .slice(-3)
        .reverse()
        .map((s) => s.source),
