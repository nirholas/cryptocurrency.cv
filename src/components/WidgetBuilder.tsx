...
const QUICK_START_TEMPLATES: QuickStartTemplate[] = [
  {
    ...
    icon: "📝",
    config: { type: "news", count: 5, width: "fixed", fixedWidth: 320, showTitle: true, showBranding: false, theme: "auto", borderRadius: 12 },
  },
  {
    name: "Homepage Hero",
    description: "Full-width price ticker for hero sections",
    icon: "🏠",
    config: { type: "ticker", width: "responsive", theme: "dark", showBranding: false, borderRadius: 0 },
  },
  {
    name: "BTC Dashboard",
    description: "Bitcoin price card with chart",
    icon: "₿",
    config: { type: "coin", coin: "bitcoin", theme: "dark", width: "fixed", fixedWidth: 400, borderRadius: 16, accentColor: "#f97316" },
  },
  {
    name: "Market Monitor",
    description: "Top coins overview widget",
    icon: "📊",
    config: { type: "market", theme: "dark", width: "responsive", showTitle: true, showBranding: true, borderRadius: 12 },
  },
  {
    name: "Sentiment Gauge",
    description: "Fear & Greed index for analyst sites",
    icon: "🎯",
    config: { type: "fear-greed", theme: "auto", width: "fixed", fixedWidth: 360, showTitle: true, borderRadius: 16, accentColor: "#22c55e" },
  },
  {
    name: "Trading View",
    description: "Interactive chart for trading pages",
    icon: "📉",
    config: { type: "chart", symbol: "BINANCE:BTCUSDT", interval: "D", theme: "dark", width: "responsive", borderRadius: 8 },
  },
];
...
export default function WidgetBuilder() {
  ...
  const embedUrl = useMemo(() => {
    ...
    if (config.accentColor !== "#3b82f6") params.set("accent", config.accentColor.replace("#", ""));
    if (config.borderRadius !== 8) params.set("radius", config.borderRadius.toString());
    if (config.refreshInterval !== 60) params.set("refresh", config.refreshInterval.toString());
  ...
  const scriptCode = `<script src="${BASE_URL}/widget/embed.js" data-type="${config.type}" data-theme="${config.theme}"${config.type === "news" ? ` data-count="${config.count}"` : ""}${config.type === "coin" ? ` data-coin="${config.coin}"` : ""}${config.type === "chart" ? ` data-symbol="${config.symbol}" data-interval="${config.interval}"` : ""}${!config.showTitle ? ' data-title="false"' : ""}${!config.showBranding ? ' data-branding="false"' : ""}${config.accentColor !== "#3b82f6" ? ` data-accent="${config.accentColor.replace("#", "")}"` : ""}${config.borderRadius !== 8 ? ` data-radius="${config.borderRadius}"` : ""}><\/script>`;
...
export default function MyWidget() {
  ...
  <crypto-widget
    type="${config.type}"
    theme="${config.theme}"${config.type === "news" ? `\n    :count="${config.count}"` : ""}${config.type === "coin" ? `\n    coin="${config.coin}"` : ""}${config.type === "chart" ? `\n    symbol="${config.symbol}"\n    interval="${config.interval}"` : ""}${!config.showTitle ? '\n    :show-title="false"' : ""}${!config.showBranding ? '\n    :show-branding="false"' : ""}${config.accentColor !== "#3b82f6" ? `\n    accent-color="${config.accentColor}"` : ""}${config.borderRadius !== 8 ? `\n    :border-radius="${config.borderRadius}"` : ""}
  />
</template>
...
<\/script>`;
  ...
  const wordpressCode = `[crypto_widget type="${config.type}" theme="${config.theme}"${config.type === "news" ? ` count="${config.count}"` : ""}${config.type === "coin" ? ` coin="${config.coin}"` : ""}${config.type === "chart" ? ` symbol="${config.symbol}" interval="${config.interval}"` : ""}${!config.showTitle ? ' title="false"' : ""}${!config.showBranding ? ' branding="false"' : ""}${config.accentColor !== "#3b82f6" ? ` accent="${config.accentColor.replace("#", "")}"` : ""}${config.borderRadius !== 8 ? ` radius="${config.borderRadius}"` : ""}]`;
  ...
  const currentWidget = WIDGET_TYPES.find((w) => w.id === config.type) ?? WIDGET_TYPES[0];
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-[var(--color-text-tertiary)] text-sm mt-3 max-w-2xl">
            Configure your widget, preview it live, and grab the embed code. Works everywhere — WordPress, React, Vue, static HTML, and more. No API key required.
          </p>
        </div>
          ...
          <div className="mb-8 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-[var(--color-text-tertiary)] mt-0.5">Choose a pre-configured template or start from scratch</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {QUICK_START_TEMPLATES.map((template) => (
                ...
                >
                  <span className="text-2xl mt-0.5 group-hover:scale-110 transition-transform">{template.icon}</span>
                  <div>
                    <div className="font-semibold text-sm">{template.name}</div>
                    <div className="text-xs text-[var(--color-text-tertiary)] mt-0.5">{template.description}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
                ...
                {WIDGET_TYPES.map((wt) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-[var(--color-text-tertiary)] mt-0.5">{wt.description}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
              ...
              <div className="space-y-5">
                {/* Theme */}
                <div>
                  <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Theme</span>
                ...
                {/* Accent Color */}
                <div>
                  <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Accent Color</span>
                ...
                {/* Width */}
                <div>
                  <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Width</span>
                  <div className="flex gap-2">
                  ...
                  {config.width === "fixed" && (
                    <div className="mt-3">
                      <input
                        ...
                        className="w-full accent-blue-500"
                      />
                      <div className="text-xs text-[var(--color-text-tertiary)] mt-1">{config.fixedWidth}px</div>
                    </div>
                  )}
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-[var(--color-text-secondary)]">Transparent background</span>
                </label>
              </div>
            </div>
            ...
            {(config.type === "news" || config.type === "coin" || config.type === "chart") && (
              <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
                ...
                <div className="space-y-4">
                  {config.type === "news" && (
                    <div>
                      <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Number of Articles</span>
                      <div className="flex gap-2">
                        {COUNT_OPTIONS.map((c) => (
                          <button
                            key={c}
                  ...
                  {config.type === "coin" && (
                    <div>
                      <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Select Coin</span>
                      <select
                        value={config.coin}
                  ...
                  {config.type === "chart" && (
                    <>
                      <div>
                        <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Trading Pair</span>
                        <select
                          value={config.symbol}
                      ...
                      <div>
                        <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Default Interval</span>
                        <div className="flex flex-wrap gap-2">
                          {[
                            { label: "1m", value: "1" },
                            { label: "5m", value: "5" },
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
              <h2 className="text-lg font-semibold mb-4">Display &amp; Behavior</h2>
              <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 rounded accent-blue-500"
                    />
                    <span className="text-sm text-[var(--color-text-secondary)]">Show &quot;Powered by&quot; branding</span>
                  </label>
                </div>

                {/* Auto-refresh */}
                <div>
                  <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Auto-refresh Interval</span>
                  <div className="flex flex-wrap gap-2">
                ...
                {/* Locale */}
                <div>
                  <span className="text-sm font-medium mb-2 block text-[var(--color-text-secondary)]">Language</span>
                  <select
                    value={config.locale}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 lg:sticky lg:top-20">
              ...
              <div className="flex justify-center">
                ...
                >
                  <iframe
                    src={embedUrl}
                    width={previewWidth ? Math.min(previewWidth, parseInt(iframeWidth) || previewWidth) : iframeWidth}
                    height={iframeHeight}
                    style={{
                      border: "none",
                      display: "block",
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-[var(--color-text-tertiary)]">
                <div className="flex items-center gap-3">
                  <span>{currentWidget.icon} {currentWidget.name}</span>
                  <span>•</span>
                  <span>{config.width === "fixed" ? `${config.fixedWidth}px` : "Responsive"} × {iframeHeight}px</span>
                </div>
                <span>Refreshes every {config.refreshInterval < 60 ? `${config.refreshInterval}s` : `${config.refreshInterval / 60}m`}</span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
              ...
              {embedTab === "react" && (
                <p className="text-xs text-[var(--color-text-tertiary)] mt-3">
                  Install: <code className="bg-[var(--color-surface-secondary)] px-1.5 py-0.5 rounded text-xs">npm install @nicholasgriffintn/crypto-widget-react</code>
                </p>
              )}
              {embedTab === "vue" && (
                <p className="text-xs text-[var(--color-text-tertiary)] mt-3">
                  Install: <code className="bg-[var(--color-surface-secondary)] px-1.5 py-0.5 rounded text-xs">npm install @nicholasgriffintn/crypto-widget-vue</code>
                </p>
              )}
              {embedTab === "wordpress" && (
                <p className="text-xs text-[var(--color-text-tertiary)] mt-3">
                  Paste this shortcode into any post or page. Requires the Crypto Vision News WordPress plugin.
                </p>
              )}
            </div>
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
              ...
              {activeSection === "tips" && (
                <ul className="text-sm text-[var(--color-text-secondary)] space-y-2 mt-4">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                    Use <code className="bg-[var(--color-surface-secondary)] px-1.5 py-0.5 rounded text-xs">theme=auto</code> to match your site&apos;s color scheme
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                    8 languages supported for international audiences
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                    Data powered by <a href={BASE_URL} className="text-blue-500 hover:underline" target="_blank" rel="noopener noreferrer">Crypto Vision News</a>
                  </li>
                </ul>
              )}
            </div>
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
              ...
              {activeSection === "faq" && (
                <div className="space-y-4 mt-4">
                  <div>
                    <h3 className="text-sm font-medium text-[var(--color-text-primary)]">Is this really free?</h3>
                    <p className="text-sm text-[var(--color-text-tertiary)] mt-1">Yes! All widgets are completely free with no API key required. We only ask that you keep the &quot;Powered by&quot; branding where possible.</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-[var(--color-text-primary)]">Can I use these on commercial sites?</h3>
                    <p className="text-sm text-[var(--color-text-tertiary)] mt-1">Absolutely. Widgets are free for both personal and commercial use.</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-[var(--color-text-primary)]">How often does the data update?</h3>
                    <p className="text-sm text-[var(--color-text-tertiary)] mt-1">Widgets auto-refresh based on your configured interval (30 seconds to 10 minutes). Price data is sourced from major exchanges in real-time.</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-[var(--color-text-primary)]">Can I customize the styling further?</h3>
                    <p className="text-sm text-[var(--color-text-tertiary)] mt-1">Yes — theme, accent color, border radius, and transparency are all configurable. For advanced CSS customization, use the script embed which gives more control.</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-[var(--color-text-primary)]">Do widgets work on mobile?</h3>
                    <p className="text-sm text-[var(--color-text-tertiary)] mt-1">Yes, all widgets are fully responsive. Use the preview device toggle above to see how they look on different screen sizes.</p>
                  </div>
                </div>
              )}
            </div>
